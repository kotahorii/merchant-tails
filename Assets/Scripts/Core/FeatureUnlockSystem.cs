...
namespace MerchantTails.Core
{
    ...
    public class FeatureUnlockSystem : MonoBehaviour
    {
        ...
        [SerializeField] private List<FeatureUnlock> featureUnlocks = new List<FeatureUnlock>();
        ...
        {
            var newlyUnlocked = featureUnlocks.FindAll(f =>
                f.requiredRank > previousRank && f.requiredRank <= newRank
            );
        ...
        {
            if (playerData == null) return new List<FeatureUnlock>();
        ...
        public List<FeatureUnlock> GetNextRankFeatures()
        {
            ...
            if (nextRank == playerData.CurrentRank) return new List<FeatureUnlock>(); // 既に最高ランク
