...
namespace MerchantTails.UI
{
    ...
    public class SettingsPanel : UIPanel
    {
        ...
        {
            if (autoSaveIntervalDropdown == null) return;
            
            autoSaveIntervalDropdown.ClearOptions();
            autoSaveIntervalDropdown.AddOptions(new System.Collections.Generic.List<string> 
            { 
                "1分", "3分", "5分", "10分", "15分" 
            });
        }
        ...
        private void ApplyAudioSettings()
        {
            if (audioMixer != null)
            {
                ...
                float masterDB = tempSettings.masterVolume > 0 ? 
                    Mathf.Log10(tempSettings.masterVolume) * 20 : -80f;
                float bgmDB = tempSettings.bgmVolume > 0 ? 
                    Mathf.Log10(tempSettings.bgmVolume) * 20 : -80f;
                float sfxDB = tempSettings.sfxVolume > 0 ? 
                    Mathf.Log10(tempSettings.sfxVolume) * 20 : -80f;
