...
namespace MerchantTails.UI
{
    ...
    public class MarketAnalysisPanel : UIPanel
    {
        ...
        [SerializeField] private Text marketTitleText;
        [SerializeField] private Text currentSeasonText;
        [SerializeField] private Text marketConditionText;
        
        [Header("Item Selection")]
        [SerializeField] private Transform itemTabContainer;
        [SerializeField] private GameObject itemTabPrefab;
        [SerializeField] private ItemType selectedItemType = ItemType.Fruit;
        
        [Header("Price Chart")]
        [SerializeField] private RectTransform chartContainer;
        [SerializeField] private LineRenderer priceLineRenderer;
        [SerializeField] private Text currentPriceText;
        [SerializeField] private Text priceChangeText;
        [SerializeField] private Text averagePriceText;
        
        [Header("Chart Settings")]
        [SerializeField] private int chartDataPoints = 30;
        [SerializeField] private float chartUpdateInterval = 2f;
        [SerializeField] private Color priceLineColor = Color.green;
        [SerializeField] private Color gridLineColor = new Color(0.3f, 0.3f, 0.3f, 0.5f);
        
        [Header("Market Info")]
        [SerializeField] private Text volatilityText;
        [SerializeField] private Text trendText;
        ...
        {
            if (itemTabContainer == null || itemTabPrefab == null) return;
        ...
        private void UpdatePriceInfo()
        {
            ...
            if (priceChangeText != null && priceHistory.Count > 1)
            {
                ...
                priceChangeText.text = $"<color={changeColor}>{changeSymbol}{change:F0}G ({changeSymbol}{changePercent:F1}%)</color>";
            }
        ...
        {
            if (marketConditionText == null) return;
        ...
        private void UpdatePrediction()
        {
            ...
            if (playerData == null || predictionPanel == null) return;
        ...
        {
            if (!priceHistory.ContainsKey(selectedItemType)) return;
        ...
        {
            if (!priceHistory.ContainsKey(itemType)) return 0f;
            
            var prices = priceHistory[itemType];
            if (prices.Count == 0) return 0f;
        ...
        {
            if (!priceHistory.ContainsKey(itemType)) return "不明";
            
            var prices = priceHistory[itemType];
            if (prices.Count < 10) return "データ不足";
        ...
        private string GeneratePrediction(ItemType itemType)
        {
            // ランクに応じた予測精度
            var playerData = GameManager.Instance?.GetPlayerData();
            if (playerData == null) return "予測不可";
            ...
            {
                MerchantRank.Master => $"詳細予測: {trend}が継続。{seasonalFactor}。変動率{volatility * 100f:F1}%で推移予想。",
        ...
        {
            // 取引ダイアログの表示（簡易実装）
            UIManager.Instance.ShowModal(UIType.TradeConfirmation, (confirmed) =>
            {
                if (confirmed)
                {
